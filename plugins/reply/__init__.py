...
async def _(event: Event, bot: Bot, args: Message = CommandArg()):
    ...
    user_name = await essentials.libraries.user.get_user_name(
        event, bot, reply.SENDER_NAME
    )
    if msg := args.extract_plain_text():
            ...
            if msg.lower() == "_enable":
                ...
                if len(splitted) == 1:
                    await _reply_handler.finish(
                        f"当前自动回复概率为{reply.get_reply_rate(gid) * 100}%"
                    )
                else:
        ...
        resp = reply.generate_response(msg).format(
            me=my_name, name=user_name, segment="\n"
        )
        for i in resp.split("\n"):
            await _reply_handler.send(i)
        await _reply_handler.finish()
    await _reply_handler.finish(
        reply.UNKNOWN_RESPONSE.format(me=my_name, name=user_name, segment="\n")
    )
