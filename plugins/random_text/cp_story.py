...
__plugin_meta__ = PluginMetadata(
    name="CP", description="生成cp文", usage="/<cp|cp文> <攻> <受>", config=None
)

_cp_stories: list[dict[str, str]] = get_data("cp_story")
_cp_handler = on_alconna(Alconna("cp", Args["s", str, ""]["m", str, ""]), block=True)


@_cp_handler.handle()
async def _(
    s: Query[str] = AlconnaQuery("s", ""), m: Query[str] = AlconnaQuery("m", "")
):
    s = s.result.strip()
    m = m.result.strip()
