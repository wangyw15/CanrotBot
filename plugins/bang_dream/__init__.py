...
async def _(comic_query: Query[str] = AlconnaQuery("comic_query", "random")):
    ...
    comics = bestdori.comic.get_comic_list()
    comic_id = None
...
async def _(song_query: Query[str] = AlconnaQuery("song_query", "random")):
    ...
    songs = bestdori.song.get_song_list()
    song_id = None
    ...
    info = bestdori.song.get_song_info(song_id)
    # TODO 封面（难做
    await _command.send(
    ...
    if await util.can_send_segment(Voice):
        await _command.send(Voice(url=bestdori.song.get_song_url(song_id)))
...
async def _(gacha_id: Query[str] = AlconnaQuery("gacha_id")):
    ...
    # 抽卡结果
    msg.append(Text(gacha_helper.generate_text(gacha_data)))
