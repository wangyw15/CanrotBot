...
_commercial_performance_handler = on_shell_command('commercial_performance', aliases={'演出'}, block=True)
        ...
        if args[0].lower() in ['pull']:
            ...
            for i in ids:
                ...
                with database.get_session().begin() as session:
                    session.execute(insert(data.CommercialPerformance).values(current['performance']))
                    session.execute(insert(data.PerformanceActors).values(current['actors']))
            await _commercial_performance_handler.finish('拉取完成')
        elif args[0].lower() in ['get']:
            with database.get_session().begin() as session:
                ret = session.execute(select(data.CommercialPerformance)
