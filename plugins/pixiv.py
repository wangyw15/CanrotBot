...
async def _(picture_id: Query[str] = AlconnaQuery("picture_id")):
    ...
    if picture_id.isdigit():
        data = await essentials.libraries.network.fetch_json_data(
            f"https://px2.rainchan.win/json/{picture_id}"
        )
        if data:
            if not data["error"]:
                imgurl = data["body"][0]["urls"]["original"]
                # TODO 缓存图片
                imgdata = await essentials.libraries.network.fetch_bytes_data(
                    imgurl, headers=_pixiv_headers
                )
                if imgdata:
