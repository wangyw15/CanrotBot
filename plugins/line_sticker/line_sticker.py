...
async def send_file(content: bytes, name: str, bot: Bot, event: Event):
    if isinstance(bot, ob11.Bot):
        with NamedTemporaryFile() as f:
            f.write(content)
            f.flush()
            if isinstance(event, ob11.PrivateMessageEvent) \
                    or isinstance(event, ob12.PrivateMessageEvent):
                await bot.call_api('upload_private_file', user_id=event.get_user_id(), file=f.name, name=name)
            elif isinstance(event, ob11.GroupMessageEvent) \
                    or isinstance(event, ob12.GroupMessageEvent):
