...
async def _(args: Annotated[list[str | MessageSegment], ShellCommandArgv()]):
    if args[0] in ['活动', 'event', 'events']:
        ...
        if data:
            ...
            for event in data:
                msg += f"{event['name']}\n"
                msg += f"类型: {event_type[event['type'] - 1]}\n"
                msg += f"Appeal 类型: {appeal_type[event['appealType']]}\n"
                msg += f"活动开始时间: {iso8601_to_local(event['schedule']['beginAt'])}\n"
                msg += f"活动结束时间: {iso8601_to_local(event['schedule']['endAt'])}\n"
                msg += f"页面开放时间: {iso8601_to_local(event['schedule']['pageOpenedAt'])}\n"
                msg += f"页面关闭时间: {iso8601_to_local(event['schedule']['pageClosedAt'])}\n"
                msg += f"加速开始时间: {iso8601_to_local(event['schedule']['boostBeginAt'])}\n" \
                       if event['schedule']['boostBeginAt'] else ''
                msg += f"加速结束时间: {iso8601_to_local(event['schedule']['boostEndAt'])}\n" \
                       if event['schedule']['boostEndAt'] else ''
                msg += f"活动物品: {event['item']['name']}" if event['item']['name'] else ''
                msg += '\n\n'
            await _mltd_handler.finish(msg.strip())
        else:
