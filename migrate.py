...
def convert_to_datetime(iso_format: str) -> datetime:
    ...
    return datetime(tmp.year, tmp.month, tmp.day, tmp.hour, tmp.minute, tmp.second, tmp.microsecond)
...
with database.get_session().begin() as session:
    session.execute(insert(user_data.User).values(
        [{'user_id': uid} for uid in uid_list]
    ))
    session.execute(insert(user_data.Bind).values(
...
for i in (data_path / 'economy').glob('*.json'):
    with i.open(encoding='utf-8') as f:
        data = json.load(f)
    with database.get_session().begin() as session:
        session.execute(insert(economy_data.Account).values(
            user_id=i.stem,
            balance=data['balance']
        ))
        session.execute(insert(economy_data.Record).values(
            [{
                'user_id': i.stem,
...
for i in (data_path / 'arknights').glob('*.json'):
    ...
    with database.get_session().begin() as session:
        session.execute(insert(arknights_data.Statistics).values(
            user_id=i.stem,
            three_stars=data['2'],
            four_stars=data['3'],
            five_stars=data['4'],
        ...
        if data['history']:
            session.execute(insert(arknights_data.History).values(
                [{
                    'user_id': i.stem,
                    'time': convert_to_datetime(record['time']),
                    'operators': json.dumps(record['operators'], ensure_ascii=False)
                } for record in data['history']]
            ))
...
with open(data_path / 'comment' / 'anime.json', encoding='utf-8') as f:
    ...
    with database.get_session().begin() as session:
        for title, contents in data.items():
            session.execute(insert(comment_data.Comment).values(
                [{
...
for i in (data_path / 'muse_dash').glob('*.json'):
    ...
    with database.get_session().begin() as session:
        session.execute(insert(muse_dash_data.MuseDashAccount).values(
            user_id=i.stem,
...
for i in (data_path / 'reply').glob('*.json'):
    with i.open(encoding='utf-8') as f:
        data = json.load(f)
    with database.get_session().begin() as session:
        session.execute(insert(reply_data.ReplyConfig).values(
            group_id=i.stem,
            enable=data['enabled'],
            rate=data['rate']
        ))
...
for i in (data_path / 'signin').glob('*.json'):
    with i.open(encoding='utf-8') as f:
        data = json.load(f)
    with database.get_session().begin() as session:
        session.execute(insert(signin_data.SigninRecord).values(
            user_id=i.stem,
...
for i in (data_path / 'trpg').glob('*.json'):
    with i.open(encoding='utf-8') as f:
        data = json.load(f)
    with database.get_session().begin() as session:
        session.execute(insert(trpg_data.Investigator).values(
            [{
