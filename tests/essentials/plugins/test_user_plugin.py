...
@pytest.mark.asyncio
async def test_user_plugin_help(
        app: App,
        create_bot: Callable,
...
async def test_user_plugin_info_not_registered(
    ...
    async with app.test_matcher(_user_command) as ctx:
        bot = create_bot(ctx)
        event = create_event(message="user info", user_id=TEST_PUID1)
...
async def test_user_plugin_info_registered(
    ...
    async with app.test_matcher(_user_command) as ctx:
        ...
        ctx.should_pass_permission()
        ctx.should_pass_rule()
        ctx.should_call_send(event, f"当前 puid: {TEST_PUID1}\n"
                                    f"uid: {uid}\n"
                                    f"已绑定的 puid:\n"
                                    f"{TEST_PUID1}")
        ctx.should_finished()
...
async def test_user_plugin_register_once(
    ...
    async with app.test_matcher(_user_command) as ctx:
        bot = create_bot(ctx)
        event = create_event(message="user register", user_id=TEST_PUID1)
    ...
    fake_register.assert_called_once_with(TEST_PUID1)
...
async def test_user_plugin_bind_once(
    ...
    async with app.test_matcher(_user_command) as ctx:
        bot = create_bot(ctx)
        event = create_event(message=f"user bind {TEST_PUID2}", user_id=TEST_PUID1)
...
async def test_user_plugin_bind_twice(
    ...
    async with app.test_matcher(_user_command) as ctx:
        ...
        ctx.should_pass_permission()
        ctx.should_pass_rule()
        ctx.should_call_send(event, f"此 puid 已经绑定或注册过了")
        ctx.should_finished()
...
async def test_user_plugin_unbind_once(
    ...
    async with app.test_matcher(_user_command) as ctx:
        bot = create_bot(ctx)
        event = create_event(message=f"user unbind {TEST_PUID2}", user_id=TEST_PUID1)
...
async def test_user_plugin_unbind_twice(
    ...
    async with app.test_matcher(_user_command) as ctx:
        ...
        ctx.should_pass_permission()
        ctx.should_pass_rule()
        ctx.should_call_send(event, f"此 puid 还未绑定或注册")
        ctx.should_finished()
