...
async def search_in_mediawiki(action: str, target: str, keyword: str):
    ...
    当前支持的百科如下：
    萌娘百科：可以查到所有漫画、动漫、游戏等二次元相关的内容，是二次元内容涵盖最全面的百科。如果是动漫、漫画等二次元ACG相关内容，就优先选择萌娘百科。
    神奇宝贝百科：可以查到所有神奇宝贝（宝可梦）相关的内容，是神奇宝贝相关内容涵盖最全面的百科
    ...
    client = MediaWikiClient(HOSTS_URL[target])

    if action == "search":
        data = await client.search(keyword)
        if not data["query"]["search"]:
            return "未找到相关页面"
        return json.dumps(data, ensure_ascii=False)
    elif action == "get":
        data = await client.get_wikitext(keyword)
        ...
        content = content[0 : llm_config.max_length]  # 防止内容过长
        return json.dumps(
