...
_qqmusic_id_pattern = (
    r"(?:https?:\/\/)?(?:\S+\.)?y\.qq\.com\/\S+(?:songid=|songDetail\/)(\d+)"
)


async def _send_music_card(
    bot: Bot, event: Event, music_type: typing.Literal["qq", "163"], music_id: str | int
):
    if isinstance(bot, ob11.Bot):
        await bot.send(
            event, ob11.Message(ob11.MessageSegment.music(music_type, music_id))
        )
...
async def _(
    reg: typing.Annotated[tuple[typing.Any, ...], RegexGroup()], bot: Bot, event: Event
):
    music_id = reg[0]
    if util.is_qq(bot):
...
_qqmusic_shortlink_handler = on_regex(
    r"(?:https?:\/\/)?c6\.y\.qq\.com\/base\/fcgi-bin\/u\?__=(\w+)", block=True
)
