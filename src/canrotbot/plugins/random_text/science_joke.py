...
_science_joke_handler = on_command(
    "science-joke", aliases={"理科笑话", "science_joke"}, block=True
)


@_science_joke_handler.handle()
async def _(event: Event, bot: Bot, args: Message = CommandArg()):
    name = await user.get_user_name(event, bot, "ta")
    if msg := args.extract_plain_text():
        name = msg
    await _science_joke_handler.finish(
        random.choice(_science_joke_data).format(name=name)
    )
