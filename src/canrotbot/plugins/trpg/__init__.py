...
from canrotbot.essentials.libraries import user, util
...
async def _(args: Message = CommandArg()):
    if expr := args.extract_plain_text():
        ...
        if str(result) == calculated_expr:
            ...
            await _dice_handler.finish(
                expr + " = " + calculated_expr + " = " + str(result)
            )
...
async def _(
    ...
    if len(args) == 1:
        if args[0] in investigator.basic_property_names:
            check_result, dice_result, target = investigator.property_check(
                uid, gid, args[0]
            )
            await _check_handler.finish(
                f"调查员 {card.name}\n"
                f"{args[0]}: {target}\n"
                f"d100 = {dice_result}\n"
                f"{check_result}"
